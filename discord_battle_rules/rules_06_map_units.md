...
__**2 Garrisons**__ - defensive units, so they can only be places on the lands that faction owns. If a territory with a *Garrison* stationed on it is attacked, it gets a **+20 base Garrison bonus** at the start of the battle (similarly to *[Adjacency](../discord_battle_rules/rules_02_initial_bonus_points.md)* bonuses). It’s allowed to place both your *Garrison* units on the same territory for a double *Garrison* bonus.
...
Unit bonuses are [doubled in capital sieges](../discord_battle_rules/rules_10_sieging_capitals.md).
